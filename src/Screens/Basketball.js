...
class Basketball extends Component {
  ...
  async updatePosition(nextState) {
    nextState.x = this.state.x + nextState.vx
    nextState.y = this.state.y - nextState.vy
    ...
    if (this.state.scored === null) {
      if (
        ...
        if (
          ...
          nextState.score += 1
          console.log(nextState.score,this.state.highScore)
          if (this.state.score >= this.state.highScore) {
            try {
              const jsonValue = JSON.stringify(nextState.score)
            ...
            this.setState({ highScore: nextState.score })
          }
  ...
  async update() {
    try {
      ...
      let highScore = jsonValue != null ? JSON.parse(jsonValue) : "0"
